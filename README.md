...
formulas, substituted values, assumptions and the standards basis, so nothing is
a black box, and everything exports as a PDF a reviewer can audit.
...
sign-in and **explicit** backup/restore, with no magic background sync.
...
1. Import the repo in Vercel and deploy. Build command is `npm run build`,
